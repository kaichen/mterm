...
export const loadMcpServersConfig = (): Record<string, McpServerConfig> => {
	try {
		...
		if (!config.mcpServers || Object.keys(config.mcpServers).length === 0) {
			logger.warn(
				'No MCP servers found in config, using default memory server',
			);
			return {
				memory: {
					command: 'npx',
		...
		logger.info(
			`Loaded ${Object.keys(config.mcpServers).length} MCP servers from config`,
		);
	...
	try {
		...
		for (const [serverId, serverConfig] of Object.entries(serversConfig)) {
			try {
				...
				logger.info(
					`MCP client for ${serverId} initialized with ${tools.length} tools`,
				);
			} catch (error) {
		...
		try {
			...
			if (clients.length > 0 && toolExists) {
				try {
					...
					logger.error(
						`Error calling MCP tool ${toolCall.function.name}:`,
						error,
					);
					toolResults.push({
						role: 'tool',
